...
SIZE = 5  # 5비트
POPULATION_SIZE = 4  # 유전자 개수
MUTATION_RATE = 0.25  # 돌연변이 비율. 1/4
population = []
...
class Chromosome:
    def __init__(self, g=[]):
        ...
        self.fitness = 0  # 적합도
        if self.genes.__len__() == 0:  # 염색체가 초기 상태이면 초기화를 한다.
            i = 0
            while i < SIZE:
...
def crossover(pop):
    ...
    child1 = father1.genes[:index] + mother.genes[index:]
    child2 = mother.genes[:index] + father1.genes[index:]
